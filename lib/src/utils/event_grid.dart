...
class EventGrid {
  ...
  void packEvents(List<List<EventDrawProperties>> columns,
    ...
    double padding = 4.0; // Add spacing between events
    ...
    for (int columnIndex = 0; columnIndex < columns.length; columnIndex++) {
      ...
      for (EventDrawProperties drawProperties in column) {
        ...
        drawProperties.width =
            (eventsColumnWidth * colSpan) / (columns.length) - padding;
      }
    }
  }
...
class EventDrawProperties {
  ...
  void calculateTopAndHeight(
    ...
    double spacing = 3.0; // Add vertical spacing
    top = topOffsetCalculator(HourMinute.fromDateTime(dateTime: start!));

    double calculatedHeight = topOffsetCalculator(
            HourMinute.fromDuration(duration: end!.difference(start!)),
            minimumTime: HourMinute.min) +
        1;

    double minHeight = 45; // Ensuring minimum height
    height = calculatedHeight < minHeight ? minHeight : calculatedHeight;
    height = height! + spacing; // Adding spacing between events
  }
